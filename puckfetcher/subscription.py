...
DATE_FORMAT_STRING = u"%Y%m%dT%H:%M:%S.%f"
HEADERS = {u"User-Agent": CONSTANTS.USER_AGENT}
...
class Subscription(object):
    ...
    def __init__(self, url=None, name=None, directory=None, backlog_limit=0):
        ...
        if url is None or url == u"":
            raise Error.MalformedSubscriptionError(u"No URL provided.")
        else:
            LOG.debug(u"Storing provided url '%s'.", url)
            self.url = url
            self.original_url = url

        # Maintain name of podcast.
        if name is None or name == "":
            raise Error.MalformedSubscriptionError(u"No name provided.")
        else:
            LOG.debug(u"Provided name '%s'.", name)
            self.name = name
    ...
    def decode_subscription(cls, sub_dictionary):
        ...
        attrs = [u"name", u"original_url", u"url"]

        for attr in attrs:
            if attr not in sub_dictionary.keys():
                LOG.error(u"Sub to decode is missing %s, can't continue.", attr)
                return None
            else:
                setattr(sub, attr, sub_dictionary[attr])

        sub.directory = sub_dictionary.get(u"directory", None)

        sub.backlog_limit = sub_dictionary.get(u"backlog_limit", 0)
        sub.use_title_as_filename = sub_dictionary.get(u"use_title_as_filename", None)
        sub.feed_state = _FeedState(feedstate_dict=sub_dictionary.get(u"feed_state", None))
    ...
    def encode_subscription(cls, sub):
        ...
        return {u"__type__": u"subscription",
                u"__version__": CONSTANTS.VERSION,
                u"url": sub.url,
                u"original_url": sub.original_url,
                u"directory": sub.directory,
                u"backlog_limit": sub.backlog_limit,
                u"use_title_as_filename": sub.use_title_as_filename,
                u"feed_state": sub.feed_state.as_dict(),
                u"name": sub.name}
    ...
    def parse_from_user_yaml(sub_yaml, defaults):
        ...
        if u"name" not in sub_yaml.keys():
            LOG.error(u"No name provided, name is mandatory!")
            return None

        if u"url" not in sub_yaml.keys():
            LOG.error(u"No URL provided, URL is mandatory!")
            return None

        sub.name = sub_yaml[u"name"]
        sub.url = sub_yaml[u"url"]
        sub.original_url = sub_yaml[u"url"]
        sub.directory = sub_yaml.get(u"directory", os.path.join(defaults[u"directory"], sub.name))
        sub.backlog_limit = sub_yaml.get(u"backlog_limit", defaults[u"backlog_limit"])
        sub.use_title_as_filename = sub_yaml.get(u"use_title_as_filename",
                                                 defaults[u"use_title_as_filename"])
    ...
    def attempt_update(self):
        ...
        LOG.info(u"Subscription %s got updated feed.", self.name)
        ...
        if self.feed_state.latest_entry_number is None:
            if self.backlog_limit is None:
                ...
                LOG.error(u"Invalid backlog limit %s, downloading nothing.", self.backlog_limit)
                return False

            elif self.backlog_limit > 0:
                LOG.info(u"Backlog limit provided as '%s'", self.backlog_limit)
                self.backlog_limit = Util.max_clamp(self.backlog_limit, number_feeds)
                LOG.info(u"Backlog limit clamped to '%s'", self.backlog_limit)
        ...
        if self.feed_state.latest_entry_number >= number_feeds:
            LOG.info(u"Number downloaded for %s matches feed entry count %s. Nothing to do.",
    ...
    def download_queue(self):
        ...
        msg = u"Queue for sub {} has {} entries.".format(self.name, len(self.feed_state.queue))
        LOG.info(msg)
        print(msg)
            ...
            while self.feed_state.queue:
                ...
                if self.feed_state.entries_state_dict.get(entry_num, False):
                    ...
                    if num_entry_files > 1:
                        directory = os.path.join(directory, entry[u"title"])
                        LOG.info(u"Creating directory to store %s enclosures.", num_entry_files)
                        print(u"{} enclosures for this feed entry.".format(num_entry_files))

                    for i, url in enumerate(urls):
                        if num_entry_files > 1:
                            LOG.info(u"Handling enclosure %s of %s.", i+1, num_entry_files)
                            print(u"Downloading enclosure {} of {}".format(i+1, num_entry_files))

                        LOG.info(u"Extracted url %s.", url)

                        # TODO catch errors? What if we try to save to a nonsense file?
                        dest = self._get_dest(url, entry[u"title"], directory)
                    ...
                    if one_indexed_entry_num > self.feed_state.latest_entry_number:
                        ...
                        LOG.info(u"Have downloaded %s entries for sub %s.",
    ...
    def enqueue(self, nums):
        ...
        LOG.info(u"New queue for %s: %s", self.name, list(self.feed_state.queue))
    ...
    def mark(self, nums):
        ...
        LOG.info(u"Items marked as downloaded for %s: %s", self.name, actual_nums)
        return actual_nums
    ...
    def unmark(self, nums):
        ...
        LOG.info(u"Items marked as not downloaded for %s: %s", self.name, actual_nums)
        return actual_nums
        ...
        if directory is not None:
            ...
            if self.directory != directory:
                ...
                if not os.path.isdir(self.directory):
                    LOG.debug(u"Directory %s does not exist, creating it.", directory)
    ...
    def default_missing_fields(self, settings):
        ...
        if self.backlog_limit is None:
            self.backlog_limit = settings[u"backlog_limit"]

        if self.use_title_as_filename is None:
            self.use_title_as_filename = settings[u"use_title_as_filename"]

        if not hasattr(self, u"feed_state") or self.feed_state is None:
            self.feed_state = _FeedState()
    ...
    def get_status(self, index, total_subs):
        ...
        return u"{}/{} - '{}' |{}|".format(padded_cur_num, total_subs, self.name,
    ...
    def get_details(self, index, total_subs):
        ...
        detail_lines.append(u"Status of podcast queue:")
        detail_lines.append(u"{}".format(list(self.feed_state.queue)))
        detail_lines.append(u"")
        detail_lines.append(u"Status of podcast entries:")

        entry_indicators = []
        for i in range(num_entries+1):
            if i in self.feed_state.entries_state_dict.keys():
                entry_indicators.append(u"{}+".format(str(i+1).zfill(pad_num)))
            else:
                entry_indicators.append(u"{}-".format(str(i+1).zfill(pad_num)))

        detail_lines.append(u" ".join(entry_indicators))
    ...
    def get_feed(self, attempt_count=0):
        ...
        def _helper():
            ...
            if attempt_count > MAX_RECURSIVE_ATTEMPTS:
                LOG.error(u"Too many recursive attempts (%s) to get feed for sub %s, canceling.",
                          attempt_count, self.name)
                res = UpdateResult.FAILURE

            elif self.url is None or self.url == u"":
                LOG.error(u"URL is empty , cannot get feed for sub %s.", self.name)
                res = UpdateResult.FAILURE
            ...
            if res != None:
                ...
                LOG.info(u"Getting entries (attempt %s) for subscription %s with URL %s.",
                         attempt_count, self.name, self.url)

            (parsed, code) = self._feedparser_parse_with_options()
            if code == UpdateResult.UNNEEDED:
                LOG.info(u"We have the latest feed, nothing to do.")
                return code

            elif code != UpdateResult.SUCCESS:
                LOG.error(u"Feedparser parse failed (%s), aborting.", code)
                return code

            LOG.info(u"Feedparser parse succeeded.")
            ...
            if code == UpdateResult.ATTEMPT_AGAIN:
                LOG.warning(u"Transient HTTP error, attempting again.")
                temp = self.temp_url
                ...
                LOG.error(u"Ran into HTTP error (%s), aborting.", code)
    ...
    def as_config_yaml(self):
        ...
        return {u"url": self.original_url,
                u"name": self.name,
                u"backlog_limit": self.backlog_limit,
                u"directory": self.directory}
    ...
    def _feedparser_parse_with_options(self):
        ...
        self.feed_state.etag = parsed.get(u"etag", self.feed_state.etag)
        self.feed_state.store_last_modified(parsed.get(u"modified_parsed", None))

        # Detect bozo errors (malformed RSS/ATOM feeds).
        if u"status" not in parsed and parsed.get(u"bozo", None) == 1:
            ...
            # Catch this case.
            if hasattr(parsed.bozo_exception, u"getMessage()"):
                msg = parsed.bozo_exception.getMessage()

            else:
                msg = repr(parsed.bozo_exception)

            LOG.error(u"Received bozo exception %s. Unable to retrieve feed with URL %s for %s.",
                      msg, self.url, self.name)
            return (None, UpdateResult.FAILURE)

        elif parsed.get(u"status") == requests.codes[u"NOT_MODIFIED"]:
            LOG.debug(u"No update to feed, nothing to do.")
    ...
    def _handle_http_codes(self, parsed):
        ...
        if u"status" not in parsed:
            LOG.info(u"Saw status 200 - OK, all is well.")
            return UpdateResult.SUCCESS

        status = parsed.get(u"status", 200)
        result = UpdateResult.SUCCESS
        if status == requests.codes[u"NOT_FOUND"]:
            LOG.error(
                textwrap.dedent(
                    u"""\
        ...
        elif status in [requests.codes[u"UNAUTHORIZED"], requests.codes[u"GONE"]]:
            LOG.error(
                textwrap.dedent(
                    u"""\
                    Saw status %s, unable to retrieve feed text for %s.
        ...
        # Handle redirecting errors
        elif status in [requests.codes[u"MOVED_PERMANENTLY"],
                       requests.codes[u"PERMANENT_REDIRECT"]]:
            LOG.warning(
                textwrap.dedent(
                    u"""\
                    ...
                    """),
                status, self.url, self.name, parsed.get(u"href"))

            self.url = parsed.get(u"href")
            result = UpdateResult.ATTEMPT_AGAIN

        elif status in [requests.codes[u"FOUND"], requests.codes[u"SEE_OTHER"],
                        requests.codes[u"TEMPORARY_REDIRECT"]]:
            LOG.warning(
                textwrap.dedent(
                    u"""\
                    ...
                    """),
                status, parsed.get(u"href"), self.url, self.name)
            ...
            LOG.warning(u"Saw status %s. Attempting retrieve with URL %s for %s again.",
                        status, self.url, self.name)
            result = UpdateResult.ATTEMPT_AGAIN

        else:
            LOG.info(u"Saw status 200. Success!")
    ...
    def _get_dest(self, url, title, directory):
        ...
        if platform.system() == u"Windows":
            ...
            filename = u"{}.{}".format(title, ext) # It's an owl!
...
class _FeedState(object):
    def __init__(self, feedstate_dict=None):
        if feedstate_dict is not None:
            LOG.info(u"Successfully loaded feed state dict.")

            self.feed = feedstate_dict.get(u"feed", {})
            self.entries = feedstate_dict.get(u"entries", [])
            self.entries_state_dict = feedstate_dict.get(u"entries_state_dict", {})
            self.queue = deque(feedstate_dict.get(u"queue", []))

            last_modified = feedstate_dict.get(u"last_modified", None)
            self.store_last_modified(last_modified)

            self.etag = feedstate_dict.get(u"etag", None)
            self.latest_entry_number = feedstate_dict.get(u"latest_entry_number", None)

        else:
            LOG.info(u"Did not successfully load feed state dict.")
    ...
    def load_rss_info(self, parsed):
        ...
        self.entries = []
        for entry in parsed.get(u"entries"):
            new_entry = {}
            new_entry[u"title"] = entry[u"title"]

            new_entry[u"urls"] = []
            for enclosure in entry[u"enclosures"]:
                new_entry[u"urls"].append(enclosure[u"href"])
    ...
    def as_dict(self):
        ...
        return {u"entries": self.entries,
                u"entries_state_dict": self.entries_state_dict,
                u"queue": list(self.queue),
                u"latest_entry_number": self.latest_entry_number,
    ...
    def store_last_modified(self, last_modified):
        ...
        if isinstance(last_modified, time.struct_time):
            LOG.info(u"Updated last_modified.")
            self.last_modified = datetime.fromtimestamp(mktime(last_modified))
        else:
            LOG.warning(u"Unhandled type, ignoring.")
            self.last_modified = None
...
def _process_directory(directory):
    ...
    if directory is None:
        LOG.debug(u"No directory provided, defaulting to %s.", directory)
        return Util.expand(CONSTANTS.APPDIRS.user_data_dir)

    LOG.debug(u"Provided directory %s.", directory)
