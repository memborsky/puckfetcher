...
def test_empty_url_cons(strdir):
    ...
    with pytest.raises(PE.MalformedSubscriptionError) as exception:
        SUB.Subscription(url=u"", name=u"emptyConstruction", directory=strdir)

    assert exception.value.desc == u"No URL provided."
...
def test_none_url_cons(strdir):
    ...
    """
    with pytest.raises(PE.MalformedSubscriptionError) as exception:
        SUB.Subscription(name=u"noneConstruction", directory=strdir)
...
def test_empty_name_cons(strdir):
    ...
    """
    with pytest.raises(PE.MalformedSubscriptionError) as exception:
        SUB.Subscription(url=u"foo", name=u"", directory=strdir)

    assert exception.value.desc == u"No name provided."
...
def test_none_name_cons(strdir):
    ...
    """
    with pytest.raises(PE.MalformedSubscriptionError) as exception:
        SUB.Subscription(url=u"foo", name=None, directory=strdir)

    assert exception.value.desc == u"No name provided."

def test_get_feed_max(strdir):
    """If we try more than MAX_RECURSIVE_ATTEMPTS to retrieve a URL, we should fail."""
    test_sub = SUB.Subscription(url=PERM_REDIRECT, name=u"tooManyAttemptsTest", directory=strdir)
...
def test_temporary_redirect(strdir):
    ...
    """
    _test_url_helper(strdir, TEMP_REDIRECT, u"302Test", TEMP_REDIRECT, TEMP_REDIRECT)
...
def test_permanent_redirect(strdir):
    ...
    """
    _test_url_helper(strdir, PERM_REDIRECT, u"301Test", RSS_ADDRESS, PERM_REDIRECT)

def test_not_found_fails(strdir):
    """If the URL is Not Found, we should not change the saved URL."""
    _test_url_helper(strdir, NOT_FOUND, u"404Test", NOT_FOUND, NOT_FOUND)

def test_gone_fails(strdir):
    """If the URL is Gone, the current url should be set to None, and we should return None."""

    test_sub = SUB.Subscription(url=GONE, name=u"410Test", directory=strdir)
...
def test_new_attempt_update(strdir):
    ...
    test_dir = strdir
    test_sub = SUB.Subscription(url=u"foo", name=u"foo", directory=test_dir)
    test_sub.downloader = generate_fake_downloader()
    test_sub.parser = generate_feedparser()
...
def test_attempt_update_new_entry(strdir):
    ...
    test_dir = strdir
    test_sub = SUB.Subscription(url=RSS_ADDRESS, name=u"bar", directory=test_dir)
    test_sub.downloader = generate_fake_downloader()
    test_sub.parser = generate_feedparser()
...
def test_attempt_download_backlog(strdir):
    ...
    test_sub = SUB.Subscription(url=RSS_ADDRESS, name=u"testfeed", directory=strdir)
    test_sub.downloader = generate_fake_downloader()
    test_sub.parser = generate_feedparser()
...
def test_attempt_download_partial_backlog(strdir):
    ...
    test_sub = SUB.Subscription(url=RSS_ADDRESS, name=u"testfeed", backlog_limit=5, directory=strdir)
...
def test_url_with_qparams():
    ...
    test_sub = SUB.Subscription(url=u"test", name=u"test", directory=u"test")

    test_sub.use_title_as_filename = True

    # pylint: disable=protected-access
    filename = test_sub._get_dest(u"https://www.example.com?foo=1/bar.mp3?baz=2", "puck", "/test")
    assert filename == u"/test/puck.mp3"

    test_sub.use_title_as_filename = False

    # pylint: disable=protected-access
    filename = test_sub._get_dest(u"https://www.example.com?foo=1/bar.mp3?baz=2", "puck", "/test")
    assert filename == u"/test/bar.mp3"
...
def test_url_sanitize():
    ...
    # pylint: disable=protected-access
    filename = test_sub._get_dest(u"https://www.example.com?foo=1/bar.mp3?baz=2", u"p/////uck",
                                  u"/test")
...
def _check_hi_contents(filename_num, directory):
    file_path = os.path.join(directory, u"hi0{}.txt".format(filename_num))
    with open(file_path, u"r") as enclosure:
...
def generate_feedparser():
    ...
    def _fake_parser(url, etag, last_modified):
        ...
        for i in range(0, 10):
            ...
            entry[u"enclosures"] = [{u"href": u"hi0{}.txt".format(i)}]
        ...
        fake_parsed[u"entries"] = entries
        fake_parsed[u"href"] = href
        fake_parsed[u"status"] = status
...
def strdir(tmpdir):
    ...
    return str(tmpdir.mkdir(u"foo"))


@pytest.fixture(scope=u"function")
def sub(strdir):
    """Create a test subscription."""
    test_sub = SUB.Subscription(url=u"test", name=u"test", directory=strdir)
